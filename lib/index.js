...
export const calcPlayerPrice = (player, devalue) => {
  if(devalue) {
    let price = ((player.aim + player.HS + player.movement + player.aggression + player.ACS + player.gamesense) * 500) * 0.1
    return parseInt(price.toString())
  }
  let price = ((player.aim + player.HS + player.movement + player.aggression + player.ACS + player.gamesense) * 500)
  return parseInt(price.toString())
}
